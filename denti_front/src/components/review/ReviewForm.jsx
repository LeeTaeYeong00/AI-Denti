...
import { createReview, updateReview, uploadReviewImages } from "../../api/reviewAPI";

// 리뷰 등록과 수정을 담당하는 입력 폼이다.
// review가 있으면 수정, 없으면 신규 등록으로 동작한다.
function ReviewForm({ reservationId, currentUserId, review, onSuccess, onCancel }) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (isUpdate) {
                response = await updateReview(review.reviewId, currentUserId, requestData);
            } else {
                response = await createReview(currentUserId, {
            ...
            if (files.length > 0) {
                try {
                    ...
                    savedReview = {
                        ...savedReview,
                        images: [...(savedReview.images ?? []), ...imageResponse.data],
                    };
                } catch (imageError) {
                    console.error("이미지 업로드 실패:", imageError);

                    alert("리뷰는 저장되었지만 이미지 업로드에 실패했습니다.");
                }
            }
            ...
            setError(responseMessage || "리뷰 저장에 실패했습니다.");
        } finally {
            setSubmitting(false);
        }
    };
        ...
        <form onSubmit={handleSubmit} className="card">
            ...
            <div style={{ display: "flex", gap: 8 }}>
                <button type="submit" className="btn btn-primary" disabled={submitting}>
                    {submitting ? "저장 중..." : isUpdate ? "수정 완료" : "리뷰 등록"}
                </button>
